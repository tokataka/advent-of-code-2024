...
pub fn solution(lines: Vec<&str>) -> String {
    ...
    for (i, line) in map.iter().enumerate() {
        for (j, &ch) in line.iter().enumerate() {
            ...
            while let Some(&(d, border)) = borders.iter().next() {
                let mut st = vec![border];

                while let Some(cur) = st.pop() {
                    if !borders.contains(&(d, cur)) {
                        continue;
                    }

                    borders.remove(&(d, cur));
                    ...
                    .for_each(|(di, dj)| {
                        match (
                            ...
                            (Some(i), Some(j)) if i < height && j < width => {
                                st.push((i, j));
                            }
                            _ => {}
                        }
