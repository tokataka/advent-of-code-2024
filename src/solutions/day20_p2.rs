...
pub fn solution(lines: Vec<&str>) -> String {
    ...
    let least_picoseconds = if cfg!(test) { 50 } else { 100 };
    ...
    while let Some((cur_i, cur_j)) = queue.pop_front() {
        ...
        for cheat_di in -20i32..=20i32 {
            for cheat_dj in -20i32..=20i32 {
                if cheat_di.abs() + cheat_dj.abs() > 20 {
                    continue;
                }
